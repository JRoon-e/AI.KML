...
def generate_circle_coords(center, radius, height, num_points=36):
    lat, lon = center
    coords = []
    for i in range(num_points):
        ...
        coords.append((point_lon, point_lat, height))
    coords.append(coords[0])  # Close the polygon
    return coords

def generate_square_coords(center, side_length, height):
    lat, lon = center
    half_side = side_length / 2
    d_lat = half_side / 111111
    d_lon = half_side / (111111 * math.cos(math.radians(lat)))
    return [
        (lon - d_lon, lat + d_lat, height),
        (lon + d_lon, lat + d_lat, height),
        (lon + d_lon, lat - d_lat, height),
        (lon - d_lon, lat - d_lat, height),
        (lon - d_lon, lat + d_lat, height)  # Close the polygon
    ]

def generate_triangle_coords(center, side_length, height):
    lat, lon = center
    tri_height = side_length * (math.sqrt(3) / 2)
    d_lat = tri_height / (3 * 111111)
    d_lon = side_length / (2 * 111111 * math.cos(math.radians(lat)))
    return [
        (lon, lat + 2*d_lat, height),
        (lon - d_lon, lat - d_lat, height),
        (lon + d_lon, lat - d_lat, height),
        (lon, lat + 2*d_lat, height)  # Close the polygon
    ]
...
def create_polygon_kml(polygon_type, center, size, height, fill_color, fill_opacity, outline_color, outline_width, output_file):
    ...
    if polygon_type == "circle":
        coords = generate_circle_coords(center, size, height)
    elif polygon_type == "square":
        coords = generate_square_coords(center, size, height)
    elif polygon_type == "triangle":
        coords = generate_triangle_coords(center, size, height)
    else:
